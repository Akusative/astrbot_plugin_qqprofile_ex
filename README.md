...
在原版手动指令的基础上，新增了 **AI 自主行为工具**、**冷却时间系统**、**AI 行为引导词注入** 和 **定时状态切换** 功能，让 Bot 能够根据对话情境自动调整自己的 QQ 资料。
...
冷却时间均可在配置面板中自由调节，设为 0 则不限制。AI 在冷却期间调用工具时会收到剩余等待时间的提示。

### AI 行为引导词注入

- 通过 `on_llm_request` 钩子，在每次 LLM 请求前自动将配置中的行为引导词注入到 system prompt
- 引导词内容可在配置面板的文本框中自由编辑，无需修改人设文件
- 预置了一套通用的行为引导模板，开箱即用
- 留空则不注入，不影响正常对话
...
| 配置项 | 说明 | 默认值 |
|:---:|:---:|:---:|
| AI行为引导词 | 自动注入到 system prompt 的行为引导文本 | 预置模板 |
| AI自动改签名 | 允许 AI 自动修改个性签名 | 开启 |
| 签名修改冷却(分钟) | 两次自动修改签名的最小间隔 | 240 |
| AI自动改状态 | 允许 AI 自动修改在线状态 | 开启 |
| 状态修改冷却(分钟) | 两次自动修改状态的最小间隔 | 60 |
| AI自动改昵称 | 允许 AI 自动修改昵称 | 开启 |
| 昵称修改冷却(分钟) | 两次自动修改昵称的最小间隔 | 1440 |
